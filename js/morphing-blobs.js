...
class MorphingBlobs {
  ...
  interpolatePaths(path1, path2, progress) {
    ...
    // In production, use a library like Flubber for perfect morphing
    ...
    commands1.forEach((cmd1, i) => {
      ...
      const nums1 = cmd1.slice(1).trim().split(/[\s,]+/).map(Number);
      const nums2 = cmd2.slice(1).trim().split(/[\s,]+/).map(Number);
